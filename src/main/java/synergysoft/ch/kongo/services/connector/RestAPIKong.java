package synergysoft.ch.kongo.services.connector;
...
public RestAPIKong connectToSalesforce() {
...
//        String uri = baseUri + "/query?q=Select+Id+,+FirstName+,+LastName+,+Company+From+Lead+Limit+5";
        String uri = baseUri + "/query?q=Select+Id+,+name+From+Siebel_item__c+Limit+5";
...
//        leadCompany = json.getJSONArray("records").getJSONObject(i).getString("Company");
        ...
        leadFirstName = json.getJSONArray("records").getJSONObject(i).getJSONObject("attributes").getString("type");
        leadLastName = json.getJSONArray("records").getJSONObject(i).getJSONObject("attributes").getString("url");
        leadCompany = json.getJSONArray("records").getJSONObject(i).getString("Name");

        System.out.println("Siebel record is: " + i + ". \n\t\tId: " + leadId + " \n\t\ttype: " + leadFirstName + " \n\t\turl: " + leadLastName + "\n\t\tName: " + leadCompany);
        }
...
public RestAPIKong createLeads(Account account) throws Exception {
        System.out.println("\n_______________ Lead INSERT _______________");

//        String uri = baseUri + "/sobjects/Lead/";
        String uri = baseUri + "/sobjects/Siebel_item__c/";
        try {
...
//        lead.put("Company", "asagarwal.com");
        ...
        lead.put("href__c", account.getFirstName());
        lead.put("rel__c", account.getLastName());
        lead.put("name__c", account.getCompany());
...
public RestAPIKong updateLeads(Account account) throws Exception {
...
//        String uri = baseUri + "/sobjects/Lead/" + leadId;
        String uri = baseUri + "/sobjects/Siebel_item__c/" + leadId;
...
//        lead.put("LastName", "Lead --UPDATED");
        lead.put("rel__c", account.getLastName() + " Siebel_item__c --UPDATED");
        ...
        System.out.println("Siebel_item__c update NOT successfully. Status code is " + statusCode);
        throw new Exception("Siebel_item__c update NOT successfully. Status code is " + statusCode);
        }
        } catch (JSONException e) {
        System.out.println("Issue creating JSON or processing results");
        e.printStackTrace();
    ...
    public RestAPIKong deleteLeads() throws Exception {
        ...
        try {
            ...
            if (statusCode == 204) {
                ...
                System.out.println("Siebel_item__c delete NOT successful. Status code is " + statusCode);
                throw new Exception("Siebel_item__c  delete NOT successful. Status code is " + statusCode);
            }
        } catch (JSONException e) {
            System.out.println("Issue creating JSON or processing results");
            e.printStackTrace();
